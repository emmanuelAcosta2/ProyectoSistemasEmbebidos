...
static void LIS3DH_enable( void )
{
    ACC_CS_SetLow();
}

static void LIS3DH_disable( void )
{
    ACC_CS_SetHigh();
}
...
* Return		: None
*******************************************************************************/
static uint8_t LIS3DH_ReadReg( uint8_t p_reg, uint8_t* p_data )
{
    //To be completed with either I2c or SPI reading function
    //i.e. *Data = SPI_Mems_Read_Reg( Reg );  
    LIS3DH_enable();
    *p_data = SPI1_Exchange16bit(((p_reg << 8) | 0x8000));
    LIS3DH_disable();
    return 1;
}
...
* Return		: None
*******************************************************************************/
static uint8_t LIS3DH_WriteReg( uint8_t p_writeAddr, uint8_t p_data)
{
    uint8_t dataread;
    
    LIS3DH_enable();
    SPI1_Exchange16bit(((p_writeAddr << 8) & 0x7F00)+p_data);
    LIS3DH_disable();
    LIS3DH_ReadReg(p_writeAddr,&dataread);
    if (dataread == p_data)
    {
        return 1;
    }
...
status_t LIS3DH_SetSIM(LIS3DH_Sim_t sim)
{
    uint8_t value;
...
status_t LIS3DH_SetClickSrc(uint8_t status)
{
    uint8_t value;
    ...
    if( !LIS3DH_WriteReg(LIS3DH_CLICK_SRC, value) )
    {
        return MEMS_ERROR;
    }
